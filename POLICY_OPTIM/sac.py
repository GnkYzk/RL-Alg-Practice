...
class Policy(nn.Module):
    def __init__(self, state_n, action_n):
        ...
        self.fc_mu = nn.Linear(128, action_n)
        self.fc_log_std = nn.Linear(128, action_n)
...
class Sac:
    def __init__(self,env,capacity,batch_size,n_episodes,discount = 0.95,lr =1e-4,polyak = 0.95,noise_clip = 0,entropy_scale = 0.1):
        self.env=env
    ...
    def replay(self,policy_update):
        ...
        with torch.no_grad():
            mu, std = self.policy(next_states)
            eps = torch.randn_like(mu)
            next_actions = torch.tanh(mu + std * eps)
            dist = torch.distributions.Normal(mu, std)
            pre_squash_action = mu + std*eps
            log_prob = dist.log_prob(pre_squash_action) - torch.log(1 - next_actions.pow(2) + 1e-6)
            log_prob = log_prob.sum(dim=1, keepdim=True)
            next_values1 = self.target_Q1(next_states,next_actions) -self.entropy_scale * log_prob
        ...
        eps = torch.randn_like(mu)
        actions_pred = torch.tanh(mu+std *eps)
        dist = torch.distributions.Normal(mu, std)
        pre_squash_action = mu + std*eps
        log_prob = dist.log_prob(pre_squash_action) - torch.log(1 - actions_pred.pow(2) + 1e-6)
        log_prob = log_prob.sum(dim=1, keepdim=True)
        loss_policy = -(self.Q1(states, actions_pred)- self.entropy_scale *log_prob).mean()
    ...
    def train(self):
        for i_episode in range(self.n_episodes):
            ...
            while not done:
                with torch.no_grad():
                    mu,std = self.policy(torch.tensor(state,dtype=torch.float32).unsqueeze(0))#is clipped in gymnasium environment with ClipAction
                    eps = torch.randn_like(mu)
